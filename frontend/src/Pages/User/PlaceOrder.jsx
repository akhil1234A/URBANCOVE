...
const PlaceOrder = () => {
  ...
  useEffect(() => {
    const fetchCheckout = async () => {
      try {
        ...
        const res = await userAxios.get("/user/cart/checkout");

        setCheckoutItems(res.data.cartItems);
        setCheckoutTotal(res.data.total);
        setCartPriceTotal(res.data.cartPriceTotal);
        setPriceChanged(res.data.priceChanged);
      } catch (err) {
  ...
  const finalTotal = checkoutTotal + deliveryFee - discount;
    ...
    try {
      ...
      const razorpayOptions = {
        ...
        modal: {
          ...
          ondismiss: async () => {
            try {
              await userAxios.post(`/orders/create-failed`, {
                razorpayOrderId,
                cartItems: checkoutItems,
                addressId: selectedAddress,
                finalTotal,
              ...
              toast.error(
                "Payment canceled. Order created with status: Failed."
              );
              localStorage.removeItem("cartData");
  ...
  const handleApplyCoupon = async () => {
    ...
    if (discount > 0) {
      toast.error(
        "A coupon is already applied. Remove it before applying a new one."
      );
      return;
    }

    try {
      const response = await userAxios.post(`/coupons/apply`, {
        couponCode, total: checkoutTotal
      });
  ...
  const handleRemoveCoupon = async () => {
    try {
      await userAxios.post(`/coupons/remove`, { couponCode });
      setDiscount(0);
      setCouponCode("");
            ...
            {addresses.map((address) => (
              <div
                ...
                className={`border p-4 rounded cursor-pointer ${
                  selectedAddress === address._id || address.isDefault
              ...
              >
                ...
                <p className="text-sm text-gray-600">
                  {address.phoneNumber}, {address.country}
                </p>
                {address.isDefault && (
      ...
      <div className="mt-8">
        {priceChanged && (
          <p className="text-sm text-orange-600 mb-2">
            ⚠️ Prices have been updated based on latest offers
          </p>
        )}
          ...
          <div className="bg-white p-6 rounded-lg w-96">
            ...
            <div className="flex flex-col gap-4">
              ...
              {addressErrors.postcode && (
                <p className="text-red-500 text-sm">{addressErrors.postcode}</p>
              )}
              <input
                type="text"
                name="country"
